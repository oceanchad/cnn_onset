...
for item in songlist:
    ...
    x, fs = librosa.load(os.path.join(audio_dir, item+audio_format), sr=None)
    if not os.path.exists(os.path.join(onset_dir, item+'.onsets')):
        continue
    onsets = np.loadtxt(os.path.join(onset_dir, item+'.onsets'))
    hop_len = int(0.01*fs)
    # get mel spectrogram
    melgram1 = librosa.feature.melspectrogram(
        y=x, sr=fs, n_fft=1024, hop_length=hop_len, n_mels=80, fmin=27.5, fmax=16000)
    melgram2 = librosa.feature.melspectrogram(
        y=x, sr=fs, n_fft=2048, hop_length=hop_len, n_mels=80, fmin=27.5, fmax=16000)
    melgram3 = librosa.feature.melspectrogram(
        y=x, sr=fs, n_fft=4096, hop_length=hop_len, n_mels=80, fmin=27.5, fmax=16000)
    ...
    for i_chunk in range(melgram1_chunks.shape[0]):
        ...
        torch.save(torch.tensor(np.array(
            [melgram1_chunks[i_chunk], melgram2_chunks[i_chunk], melgram3_chunks[i_chunk]])), savepath)
