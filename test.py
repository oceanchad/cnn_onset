...
for song in songlist:
    ...
    for i_file in range(n_files):
        x = torch.load(os.path.join(datadir, song+'/%d.pt' %
                       i_file)).to(device)
        x = x.unsqueeze(0)
        y = model(x).squeeze().cpu().detach().numpy()
        odf = np.append(odf, y)
        gt = np.append(gt, labels[os.path.join(
            datadir, song+'/%d.pt' % i_file)])
    ...
    for predict_thresh in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8]:
        ...
        scores_thresh = np.append(scores_thresh, utils.eval_output(
            odf_labels, odf, gt, tolerance_frame, mergeDur_frame))

    # accumulate hits and misses for every song
    if len(scores) == 0:
        scores = np.atleast_2d(np.array(scores_thresh))
    else:
        scores = np.vstack((scores, np.atleast_2d(np.array(scores_thresh))))
